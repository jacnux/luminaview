...
const PortfolioPage = () => {
  ...
  // --- NOUVELLE FONCTION : Détection URL Blog ---
  const getBlogUrl = () => {
    const name = user?.name?.toLowerCase() || 'jac';
    const hostname = window.location.hostname;

    // Si on est en local, on redirige vers le blog local avec le paramètre user
    if (hostname === 'localhost') {
        return `http://localhost:8080/?user=${name}`;
    }

    // Sinon, on utilise le domaine de production
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 border-b border-gray-800 mb-10">
          <div className="flex justify-center gap-2 md:gap-4 flex-wrap">
                <button onClick={() => setActiveTab('about')} className={`px-6 py-3 text-sm font-bold rounded-t-lg transition ${activeTab === 'about' ? 'bg-gray-800 text-yellow-400 border-b-2 border-yellow-400' : 'text-gray-500 hover:text-white'}`}>
                  À propos
                </button>
              <button onClick={() => setActiveTab('projects')} className={`px-6 py-3 text-sm font-bold rounded-t-lg transition ${activeTab === 'projects' ? 'bg-gray-800 text-yellow-400 border-b-2 border-yellow-400' : 'text-gray-500 hover:text-white'}`}>
                Galeries
              </button>
              ...
              {userPages.map(p => (
                      <Link key={p._id} to={`/portfolio/${username}/${p.slug}`} className="px-6 py-3 text-sm font-bold rounded-t-lg transition text-gray-500 hover:text-white cursor-pointer">
                        {p.title}
                      </Link>
                ))}

              <button onClick={() => setActiveTab('services')} className={`px-6 py-3 text-sm font-bold rounded-t-lg transition ${activeTab === 'services' ? 'bg-gray-800 text-yellow-400 border-b-2 border-yellow-400' : 'text-gray-500 hover:text-white'}`}>
                  Mes Actualités
              </button>
          </div>
      </div>
             ...
             <div className="max-w-3xl mx-auto">
                <div className="bg-gray-800/50 backdrop-blur border border-white/10 rounded-xl p-8 shadow-2xl">
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-700 text-center">
                        <a href={`mailto:${user.email}`} className="inline-block bg-yellow-500 hover:bg-yellow-400 text-black px-8 py-3 rounded-full font-bold transition text-lg">Me contacter</a>
                    </div>
                </div>
             </div>
          )}
          {activeTab === 'services' && (
             <div className="max-w-3xl mx-auto">
                <div className="bg-gray-800/50 backdrop-blur border border-white/10 rounded-xl p-8 shadow-2xl">
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-700 text-center">
                        <a href={`mailto:${user.email}`} className="inline-block bg-yellow-500 hover:bg-yellow-400 text-black px-8 py-3 rounded-full font-bold transition text-lg">Demander un devis</a>
                    </div>
                </div>
             </div>
          )}
